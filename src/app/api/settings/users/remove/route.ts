...
export async function POST(request: Request) {
  ...
  if (userId === authUser.id) {
    return NextResponse.json(
      { error: "You cannot remove your own account from workspace." },
      { status: 400 },
    );
  }
  ...
  if (target.is_owner) {
    return NextResponse.json(
      { error: "Owner cannot be removed from workspace." },
      { status: 409 },
    );
  }
  ...
  if (updateError) {
    return NextResponse.json(
      { error: updateError.message ?? "Failed to remove user from workspace." },
      { status: 500 },
    );
  }
