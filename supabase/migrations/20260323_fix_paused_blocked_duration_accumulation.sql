...
begin
  ...
  if p_to_status not in ('pending', 'queued', 'in_progress', 'paused', 'blocked', 'done') then
  ...
  case v_run.status
    ...
    when 'queued' then
      v_transition_allowed := p_to_status in ('pending', 'in_progress', 'blocked', 'done');
    when 'in_progress' then
      v_transition_allowed := p_to_status in ('paused', 'blocked', 'done');
    when 'paused' then
      v_transition_allowed := p_to_status in ('in_progress', 'blocked', 'done');
    when 'blocked' then
      v_transition_allowed := p_to_status in ('queued', 'in_progress', 'paused', 'done');
    when 'done' then
  ...
  update public.production_items pi
     set status = p_to_status,
         started_at = case
           ...
           when p_to_status in ('queued', 'pending', 'paused', 'blocked') then null
           else pi.started_at
         end,
         done_at = case
