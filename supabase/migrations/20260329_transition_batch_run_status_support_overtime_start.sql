...
set overtime_enabled = exists (
  select 1
  from jsonb_array_elements(coalesce(ts.work_shifts, '[]'::jsonb)) as shift(value)
  where coalesce(value->>'overtimeStart', value->>'overtime_start', value->>'start')
          <> coalesce(value->>'start', '')
     or coalesce(value->>'overtimeEnd', value->>'overtime_end', value->>'end')
          <> coalesce(value->>'end', '')
)
where ts.overtime_enabled is distinct from exists (
  select 1
  from jsonb_array_elements(coalesce(ts.work_shifts, '[]'::jsonb)) as shift(value)
  where coalesce(value->>'overtimeStart', value->>'overtime_start', value->>'start')
          <> coalesce(value->>'start', '')
...
create or replace function public.compute_tenant_counted_minutes(
  p_tenant_id uuid,
  p_start timestamptz,
  p_end timestamptz
)
...
declare
  ...
  v_work_shifts jsonb := '[{"start":"08:00","end":"17:00","overtimeStart":"08:00","overtimeEnd":"17:00"}]'::jsonb;
  v_overtime_enabled boolean := false;
  v_day timestamptz;
  v_end_day timestamptz;
  v_shift jsonb;
  v_shift_start time;
  v_shift_end time;
  v_overtime_start time;
  v_overtime_end time;
  v_counted_start timestamptz;
  v_range_start timestamptz;
  v_regular_end timestamptz;
  v_counted_end timestamptz;
...
begin
  ...
  select
    ...
    coalesce(ts.overtime_enabled, false),
    coalesce(
      ts.work_shifts,
      '[{"start":"08:00","end":"17:00","overtimeStart":"08:00","overtimeEnd":"17:00"}]'::jsonb
    )
  into v_workdays, v_overtime_enabled, v_work_shifts
  ...
  while v_day <= v_end_day loop
    if extract(dow from v_day)::integer = any(v_workdays) then
      ...
      loop
        ...
        v_overtime_start := coalesce(
          (v_shift->>'overtimeStart')::time,
          (v_shift->>'overtime_start')::time,
          v_shift_start
        );
        v_overtime_end := coalesce(
          (v_shift->>'overtimeEnd')::time,
          (v_shift->>'overtime_end')::time,
          v_shift_end
        );
        ...
        v_regular_end := v_day + v_shift_end;

        if v_shift_end <= v_shift_start then
          v_regular_end := v_regular_end + interval '1 day';
        end if;

        if v_overtime_enabled then
          v_counted_start := v_day + v_overtime_start;
          while v_counted_start > v_range_start loop
            v_counted_start := v_counted_start - interval '1 day';
          end loop;

          v_counted_end := v_day + v_overtime_end;
          while v_counted_end < v_regular_end loop
            v_counted_end := v_counted_end + interval '1 day';
          end loop;
        else
          v_counted_start := v_range_start;
          v_counted_end := v_regular_end;
        end if;

        v_overlap_start := greatest(v_counted_start, p_start);
        v_overlap_end := least(v_counted_end, p_end);
...
begin
  ...
  if p_to_status = 'in_progress' then
    ...
    v_duration := coalesce(v_run.duration_minutes, 0) + greatest(
      1,
      public.compute_tenant_counted_minutes(
        v_run.tenant_id,
        v_started_at,
        v_done_at
      )
